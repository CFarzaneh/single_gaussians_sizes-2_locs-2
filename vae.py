...
def init_weights(shape, n_visible, n_hidden):
    W_init_max = 4 * np.sqrt(6. / (n_visible + n_hidden))
    return tf.random_uniform(shape=[n_visible, n_hidden],
			     minval=-W_init_max,
			     maxval=W_init_max)
...
class VariationalAutoencoder(object):
	...
	'''
	# Recognition Network (Encoder) NO Xaviar Init
	all_weights['weights_recog'] = {}
	all_weights['weights_recog']['hidden_layers'] = []
        recogW = all_weights['weights_recog']['hidden_layers']
        all_weights['biases_recog'] = {}
        ...
        prevNodes = n_input
        for numNodes in n_hidden_recog:
		recogW.append(tf.Variable(init_weights(
	'''
